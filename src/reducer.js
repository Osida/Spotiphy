// initial data layer state
export const initialState = {
  user: null,
  playlist: [],
  playing: false,
  item: null,
  // Remove after finished developing
  token:
    "BQAsxfBQYSnG3LdI0zpiNB1SwUZstisjq_WbwrP2krlSikHPbeaRdL02kg_I_gm6DZgHLco7QjNmdtX5r97YHE63y6ja3Q62om23HjPSIfk40Zew2BT9_pP30Hznkl-gQse7q3k7nyVL_WsTQ6NTIFdOfBIat9QhSxpIRYDhA-ph2HpwtkwX3DOXryNaK_bvYvEKEfgGvJPGse72wnIwCris6Btp_Flj1QfInrJQ8_gOoqEU_RlTWpm1zIpL8Do43DHm0YYzB8vIDSNgRS1i0Xp0Boue3Zo",
};

// state = how the data layer currently looks
// action = how we manipulate how the data layer looks like

// reducers primary job is to listen to/for actions
// action has 2 things: action -> type, [payload] *can be called whatever we want
const reducer = (state, action) => {
  console.log("action = ", action);

  switch (action.type) {
    case "SET_USER":
      // this is what the new state will look like
      return {
        ...state,
        user: action.user,
      };
    case "SET_TOKEN":
      return {
        ...state,
        token: action.token,
      };
    default:
      return state;
  }
};

export default reducer;
